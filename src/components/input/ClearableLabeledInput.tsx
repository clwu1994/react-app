 ...
 * 此props仅供input
 */
interface ClearableInputProps extends BaseProps {
...
<     span className={`${prefixCls}-prefix`}>{props.prefix}</span>
  ...
  renderInputWithLabel(prefixCls: string, labeledElement: React.ReactElement<any>) {
    ...
    const mergeWrapperClassName = classNames(`${prefixCls}-wrapper`, {
    ...
    // 需要另一个wrapper将display：table，更改为display：inline-block
    // 然后将style prop放入wrapper中
    return (
      <span className={mergedGroupClassName} style={style}>
        <span className={mergeWrapperClassName}>
          {addonBeforeNode}
